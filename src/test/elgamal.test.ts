...
describe("Testing ElGamal Scheme on EC points directly", async () => {
    it("Check compliance of orignal and decrypted message as points", async () => {
        ...
        const decrypted_message = await decrypt(keypair.private_key,  encryption.ephemeral_key, encryption.encrypted_message);
    ...
    it("Check unhappy compliance of orignal and decrypted message as points", async () => {
        ...
        const decrypted_message = decrypt(keypair.private_key,  encryption.ephemeral_key, encryption.encrypted_message);
    ...
    it("Check LOOPED compliance of orignal and decrypted message as points", async () => {
        ...
        for (let i=0; i<10; i++) {
            ...
            let encryption = await encrypt(keypair.public_key);
            let decrypted_message = await decrypt(keypair.private_key,  encryption.ephemeral_key, encryption.encrypted_message);
            
            expect(encryption.message,'Decrypted message is different!').to.deep.equal(decrypted_message);
        }    
    });
    ...
    it("Check homomorphic properties of the Elgamal Scheme", async () => {
            ...
            const encrypted_message3 = babyjub.addPoint(encryption1.encrypted_message, encryption2.encrypted_message);  
            const ephemeral_key3 = babyjub.addPoint(encryption1.ephemeral_key, encryption2.ephemeral_key);
            const decrypted_message3 = await decrypt(keypair.private_key, ephemeral_key3, encrypted_message3);    
    ...
    it("Check unhappy homomorphic properties for wrong inputs", async () => {
        ...
        const ephemeral_key3 = babyjub.addPoint(encryption1.ephemeral_key, encryption2.ephemeral_key).reverse();
        const decrypted_message3 = await decrypt(keypair.private_key, ephemeral_key3, encrypted_message3);    
...
describe.skip("Testing Encoding/Decoding for ElGamal Scheme", async () => {
    ...
    it("Check decoding preserves Elgamal linear homomorphism", async () => {
        ...
        const decrypted_message1 = await decrypt(keypair.private_key,  encryption1.ephemeral_key, encryption1.encrypted_message);
        const decrypted_message2 = await decrypt(keypair.private_key,  encryption2.ephemeral_key, encryption2.encrypted_message);
