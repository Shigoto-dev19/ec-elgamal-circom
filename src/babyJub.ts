...
export function getInRange(min: bigint ,max: bigint) : bigint {
    ...
    const bytes_num = Math.floor((range.toString(2).length - 1) / 8);       // calculate the number of random bytes in the range
    let bi = undefined;
  
    do {
        const buffer = crypto.randomBytes(bytes_num);                       // generate random bytes according to the number of bytes in range
...
export async function encrypt(public_key) {
    ...
    const nonce = getInRange(BigInt(1), babyjub.order);                                                             // The sender chooses a secret key as a nonce k
